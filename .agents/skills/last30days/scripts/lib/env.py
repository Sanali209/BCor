...
def get_openai_auth(file_env: dict[str, str], settings_overrides: dict[str, Any] | None = None) -> OpenAIAuth:
...
def _resolve_secret(*values: str | None) -> str | None:
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, str | None]:
    ...
    if available == "none":
        if requested == "auto":
            ...
            return "web", "No API keys configured. Add keys to ~/.config/last30days/.env for Reddit/X."
    ...
    if available == "web":
        if requested == "auto":
            ...
            return (
                "web",
                "Only web search keys configured. Add OPENAI_API_KEY (or run codex login) for Reddit, XAI_API_KEY for X.",
            )
