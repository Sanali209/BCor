...
def get_cognitive_complexity_complexipy(path: Path) -> dict:
    ...
    try:
        ...
        for py_file in files:
            ...
            if any(
                part.startswith(".") or part in ("venv", "env", "__pycache__", "node_modules", "migrations")
            ...
            try:
                ...
                results[str(py_file)] = {"total": result.complexity, "functions": file_functions}
...
def get_cognitive_via_cli(path: Path) -> dict:
    ...
    try:
        result = subprocess.run(["complexipy", str(path), "--output-json", "--quiet"], capture_output=True, text=True)
        if result.returncode == 0 and result.stdout:
            ...
            for file_data in data.get("files", []):
                ...
                results[file_data["path"]] = {"total": file_data.get("complexity", 0), "functions": file_functions}
            return results
    except Exception as e:
...
def get_cyclomatic_complexity(file_path: Path) -> dict:
    ...
    try:
        result = subprocess.run(["radon", "cc", str(file_path), "-j"], capture_output=True, text=True, check=True)
        data = json.loads(result.stdout)
...
def get_maintainability_index(file_path: Path) -> float:
    ...
    try:
        result = subprocess.run(["radon", "mi", str(file_path), "-j"], capture_output=True, text=True, check=True)
        data = json.loads(result.stdout)
...
def analyze_file(file_path: Path, cognitive_data: dict) -> FileMetrics | None:
...
def analyze_path(target_path: Path) -> list[FileMetrics]:
    ...
    for py_file in files:
        # Skip common excludes
        if any(
            part.startswith(".") or part in ("venv", "env", "__pycache__", "node_modules") for part in py_file.parts
        ):
            continue
...
def print_report(results: list[FileMetrics], thresholds: Thresholds) -> int:
    ...
    print(f"\n{'=' * 80}")
    print("MULTI-METRIC CODE ANALYSIS REPORT")
    print("Stack: Ruff (linting) + Complexipy (cognitive) + Radon (cyclomatic/MI)")
    print(f"{'=' * 80}\n")
    ...
    violations = {
        ...
        "cognitive_warning": sum(
            1 for f, _ in all_functions if thresholds.cognitive_warning < f.cognitive <= thresholds.cognitive_error
        ),
        "cyclomatic_error": sum(1 for f, _ in all_functions if f.cyclomatic > thresholds.cyclomatic_error),
        "cyclomatic_warning": sum(
            1 for f, _ in all_functions if thresholds.cyclomatic_warning < f.cyclomatic <= thresholds.cyclomatic_error
        ),
    }
