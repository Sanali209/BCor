...
def check_flake8_installed() -> bool:
    ...
    try:
        result = subprocess.run(["flake8", "--version"], capture_output=True, text=True, timeout=5)
...
def run_flake8_analysis(
    ...
    if not check_flake8_installed():
        return {"error": "flake8 is not installed", "install_command": "pip install flake8"}
    ...
    try:
        ...
        return parse_flake8_output(result.stdout, result.stderr, result.returncode, target_path, installed_plugins)

    except subprocess.TimeoutExpired:
        return {"error": "flake8 analysis timed out (>5 minutes)", "target": str(target_path)}
    except Exception as e:
        return {"error": f"Failed to run flake8: {e}", "target": str(target_path)}


def parse_flake8_output(
    stdout: str, stderr: str, returncode: int, target_path: Path, installed_plugins: dict[str, bool]
) -> dict[str, Any]:
...
def generate_summary_report(analysis: dict[str, Any]) -> str:
    ...
    missing_recommended = [
        p for p in RECOMMENDED_PLUGINS if p not in ESSENTIAL_PLUGINS and not installed_plugins.get(p)
    ]
    ...
    if analysis["by_file"]:
        ...
        lines.append("-" * 70)
        sorted_files = sorted(analysis["by_file"].items(), key=lambda x: x[1], reverse=True)
...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive code analysis using flake8 with plugins")
    ...
    parser.add_argument("--html", type=Path, help="Output file for HTML report")
    parser.add_argument("--max-complexity", type=int, default=10, help="Maximum cyclomatic complexity (default: 10)")
    parser.add_argument(
        "--max-cognitive-complexity", type=int, default=10, help="Maximum cognitive complexity (default: 10)"
    )
    parser.add_argument("--ignore", type=str, help="Comma-separated list of error codes to ignore")
