...
def has_type_hints(node: ast.FunctionDef) -> tuple[bool, int, int]:
    ...
    num_params_with_types = sum(1 for arg in node.args.args if arg.annotation is not None)
...
def analyze_class(node: ast.ClassDef) -> ClassDocMetrics:
    ...
    methods_documented = sum(1 for m in methods if ast.get_docstring(m) is not None)
...
def analyze_file(file_path: Path) -> FileDocMetrics:
    ...
    public_functions_with_docstrings = sum(1 for f in functions if f.is_public and f.has_docstring)
    ...
    public_classes_with_docstrings = sum(1 for c in classes if c.is_public and c.has_docstring)
...
def print_metrics(metrics: FileDocMetrics, verbose: bool = True):
    ...
    if verbose:
        ...
        undocumented_funcs = [f for f in metrics.functions if f.is_public and not f.has_docstring]
        undocumented_classes = [c for c in metrics.classes if c.is_public and not c.has_docstring]
        ...
        missing_types = [
            ...
            if f.is_public and (not f.has_return_type or f.num_params_with_types < f.num_params)
        ]
...
def main():
    parser = argparse.ArgumentParser(description="Check documentation coverage for Python code")
