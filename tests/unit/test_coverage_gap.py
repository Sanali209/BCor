...
async def test_agm_handler_error_handling():
    event = StoredFieldRecalculationRequested(node_id="node1", field_name="field1", new_source_val=10.0)
...
async def test_agm_mapper_live_hydration_failure():
    ...
    class Node:
        id: str
        handler: Annotated[Any | None, Live(handler=int)] = None
...
@pytest.mark.asyncio
async def test_ecs_handler_move_entity_success() -> None:
    from src.modules.ecs.domain import PositionComponent
    from src.modules.ecs.handlers import handle_move_entity
    ...
    result = await handle_move_entity(cmd, uow)
