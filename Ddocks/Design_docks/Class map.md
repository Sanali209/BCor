...
### Vision (`vision/`)
*   **Domain**:
    *   `IVisionTagger(abc.ABC)`: Core interface for image analysis.
*   **Adapters**:
    *   `WDTaggerAdapter`: Local AI tagging (timm/torch).
*   **Registration**: `VisionModule(BaseModule)`
...
*   **Web Adapters (`web/`)**:
    *   `IBrowser`: Browser automation port.
    *   `IExtractor`: HTML extraction port.
    *   `PlaywrightAdapter`: Playwright implementation.
    *   `BS4Extractor`: BeautifulSoup4 implementation.
    *   `ResourceDownloader`: Async file downloader.
    *   `ScraperEngine`: Domain-agnostic web scraping orchestrator.
