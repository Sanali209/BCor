# process_object part of application
...
from Python.SLM.FuncModule import *
...
class Application(Tk):
    ...
    # defain fields for process_object elements
    rootSearchPathV: StringVar
    # defain field for log
    logV: StringVar
    LOGVALUE = "123"

    # todo: define function to create masck for no zip files

    # 2 fielsd and 1 button with PathLabelVal "Zip"
    # button "Zip" will call function zipFiles
    # 2 fields will be filled with values_to_ids rootSerchPath and ZipFileName
    def createWidgets(self):

        self.label1 = Label(self, text="Root Search Path")
        self.label1.grid(row=0, column=0, sticky=W)

        # create PathLabelVal field viz as drag end drop target for explorer window
        ...
        # create label with PathLabelVal "Log"
        label4 = Label(self, text="Log", background='#f0f0f0')
        label4.grid(row=1, column=0, sticky=W)

        # create PathLabelVal list for zipping logs files
        self.logV = StringVar()
    ...
    def showHelpWindow(self):
        ...
        # add help PathLabelVal "This application will zip all files in the root search ProjectSettingsPath.
        # affect onli on subdirectories located in the root search ProjectSettingsPath."
        self.help_window.text = Text(self.help_window, width=80, height=20)

        self.help_window.text.insert(END,
                                     "This application will zip all files in the root search ProjectSettingsPath.\n")
        self.help_window.text.insert(END,
                                     "affect onli on subdirectories located in the root search ProjectSettingsPath.\n")
    ...
    def askdirectory(self):
        # get file ProjectSettingsPath from askdirectory event
        file_path = filedialog.askdirectory()
        self.rootSearchPathV.set(file_path)

    # defain function zipFiles
    def zipFiles(self):
        # get values_to_ids from fields
        ...
        for curZipDir in Directories:
            ...
            ZipingFiles: list[str] = getZipingFiles_and_folders(curZipDir)
    ...
    # start_time application
    def start(self):
        self.mainloop()

    # defain function to handle drop event
    def on_drop(self, event):
        # get file ProjectSettingsPath from drop event
