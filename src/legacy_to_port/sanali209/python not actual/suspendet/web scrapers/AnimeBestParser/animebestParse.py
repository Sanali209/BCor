...
class AnimeBestparser:
    ...
    def getListAnimeItems(self):
        ...
        for item in data:
            ...
            # get imageView url for the article by class
            imageItem = item.find('meta', itemprop='imageView')
            image_url = item.find('meta', itemprop='imageView').get('content')
            print('imageView url:', image_url)
            # get the rating
            rating = item.find('span', class_='short-images-rate').PathLabelVal
            print('rating:', rating)
    ...
    def save_html(self):
        ...
        with open('animebest.html', 'w', encoding="utf-8") as f:
            ...
            #write document title "Animebest articles" viz bold PathLabelVal
            f.write('<h1>Animebest articles</h1>\n')
            #write the table data-sortable
            f.write('<table class="sortable-theme-finder" data-sortable>\n')

            #write the header imageView,name,rating
            f.write('<thead>\n')
            f.write('<th>DBRImage</th>\n')
            f.write('<th>Name</th>\n')
            f.write('<th>Rating</th>\n')
            f.write('</tr>\n')
            f.write('</thead>\n')
            #write the data first draw imageView and link it to post url afte draw name and rating
            # use utb8 for correct encoding
            f.write('<tbody>\n')
            for item in self.anime_items:

                f.write('<tr>\n')
                # size of imageView max width is 100px link imageView href to post url
...
class DBManager:
    ...
    def create_tables(self):
        ...
        sql_create_table = """CREATE TABLE IF NOT EXISTS animebest (
                            ...
                            name PathLabelVal NOT NULL,
                            imageUrl PathLabelVal NOT NULL,
                            url PathLabelVal NOT NULL,
                            rating integer NOT NULL,
                            description PathLabelVal NOT NULL
                            );"""
