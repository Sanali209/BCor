...
class AGMMapper:
    ...
    async def save(self, model: Any, previous_state: dict[str, Any] = None, session: Any = None):
                    ...
                    for idx, target_node in enumerate(nodes):
                        ...
                        if target_id:
                            ...
                            if rel_metadata.direction == "OUTGOING":
                                rel_statements.append(f"MERGE (n)-[:{rel_metadata.type}]->(:{target_label} {{id: ${param_name}}})")
                            else:
                                rel_statements.append(f"MERGE (n)<-[:{rel_metadata.type}]-(:{target_label} {{id: ${param_name}}})")
            else:
        ...
        # 2. Side Effects (TaskIQ for stored fields)
                ...
                for metadata in get_args(field_type)[1:]:
                    if isinstance(metadata, Stored):
                        ...
                        if new_source_val != old_source_val:
                            logger.info(f"Source field '{source_field}' changed. Triggering async calculation for '{field_name}'.")
                            try:
                                await tasks.compute_stored_field.kiq(model.id, field_name, new_source_val)
                            except Exception as e:
                                logger.error(f"Failed to dispatch TaskIQ job for '{field_name}': {e}")
