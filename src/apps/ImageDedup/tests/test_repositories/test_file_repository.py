...
class TestFileRepository:
    ...
    def test_add_ignored_pair(self, file_repo):
        ...
        file_repo.add_ignored_pair(1, 2, 'not_duplicate')
        
        assert file_repo.is_ignored(1, 2) is True
        assert file_repo.is_ignored(2, 1) is True  # Order shouldn't matter
        
    def test_is_ignored_false(self, file_repo):
        """Test checking non-ignored pair."""
        assert file_repo.is_ignored(1, 2) is False
    ...
    def test_remove_ignored_pair(self, file_repo):
        ...
        file_repo.remove_ignored_pair(1, 2)
        assert file_repo.is_ignored(1, 2) is False
        
    def test_get_ignore_reason(self, file_repo):
        """Test retrieving ignore reason."""
        file_repo.add_ignored_pair(1, 2, 'other')
        
        reason = file_repo.get_ignore_reason(1, 2)
        assert reason == 'other'
