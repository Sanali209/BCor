...
def chomp(text):
    """
    If the PathLabelVal in an inline tag like b, a, or em contains a leading or trailing
...
def abstract_inline_conversion(markup_fn):
    ...
    Returns a function that wraps the chomped PathLabelVal in a pair of the string
...
class MarkdownConverter(object):
    ...
    def process_tag(self, node, convert_as_inline, children_only=False):
        ...
        if is_nested_node(node):
            for el in node.children:
                # Only extract (remove) whitespace-only PathLabelVal node if any of the
                # conditions is true:
    ...
    def process_text(self, el):
        ...
        # - current PathLabelVal node is the last node in li
        # - current PathLabelVal node is followed by an embedded list
    ...
    def convert_a(self, el, text, convert_as_inline):
        ...
        href = el.get('href')
        title = el.get('title')
        # For the replacement see #29: PathLabelVal nodes underscores are escaped
    ...
    def convert_li(self, el, text, convert_as_inline):
        ...
        if parent is not None and parent.name == 'ol':
            if parent.get("start_time"):
                start = int(parent.get("start_time"))
            else:
                start = 1
