...
class FieldConfig:
    ...
    exclude_extensions: Optional[List[str]] = field(default_factory=lambda: ["gif"]) # Список исключеных расширений для ресурсов
    ...
    def load_from_file(cls, filepath: str = "config.json") -> 'ProjectConfig':
        try:
            ...
            if "image_save_path_pattern" in config_data:
                ...
                logger.warning("Обнаружен устаревший ключ 'image_save_path_pattern' в config.json. Используйте 'resource_save_path_pattern'.")
            if "overwrite_images" in config_data:
                 config_data["overwrite_resources"] = config_data.pop("overwrite_images")
                 logger.warning("Обнаружен устаревший ключ 'overwrite_images' в config.json. Используйте 'overwrite_resources'.")
            ...
            if "{ext}" not in config_data["resource_save_path_pattern"]:
                 logger.warning("Паттерн 'resource_save_path_pattern' не содержит '{ext}'. Расширение файла может быть некорректным.")
        ...
        try:
            ...
            try:
                relative_path = self.config.resource_save_path_pattern.format(**format_data)
                 ...
                 logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Ошибка в resource_save_path_pattern: ключ {e} не найден. URL: {resource_url}")
                 return None
            except Exception as e:
                 logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Неожиданная ошибка форматирования пути: {e}. URL: {resource_url}. Данные: {format_data}")
                 return None
            ...
            if not self.config.overwrite_resources and os.path.exists(absolute_path):
                logger.debug(f"[{topic_data.topic_id}/{resource_field_name}] Ресурс уже существует ({relative_path}), overwrite_resources=False.")
            ...
            logger.debug(f"[{topic_data.topic_id}/{resource_field_name}] Попытка скачивания ресурса через Playwright Sync API: {resource_url}")
            ...
            try:
                ...
                if not response.ok:
                    logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Playwright Sync API request failed for {resource_url} with status {response.status}")
                ...
                if not body_bytes:
                     logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Получено пустое тело ответа для: {resource_url}")
                     return None

                logger.debug(f"Успешно получены байты для {resource_url} (Content-Type: {content_type}, Size: {len(body_bytes)})")

            except PlaywrightTimeoutError:
                 logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Таймаут ({self.config.download_timeout_ms}ms) при скачивании ресурса: {resource_url}")
                 return None
            except PlaywrightError as e:
                 logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Playwright Sync API request error for {resource_url}: {e}")
                 return None
            except Exception as e: # Ловим и другие возможные ошибки запроса (e.g., network issues)
                 logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Unexpected Sync API request error for {resource_url}: {e}")
                 return None
            ...
            if final_ext != url_ext:
                 logger.debug(f"[{topic_data.topic_id}/{resource_field_name}] Расширение уточнено: {url_ext} -> {final_ext} (Content-Type: {content_type})")
                 format_data["ext"] = final_ext
                 try:
                    relative_path = self.config.resource_save_path_pattern.format(**format_data)
                 except KeyError as e:
                     logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Ошибка в resource_save_path_pattern (уточн. ext): ключ {e} не найден. URL: {resource_url}")
                     return None
                 except Exception as e:
                     logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Неожиданная ошибка форматирования пути (уточн. ext): {e}. URL: {resource_url}. Данные: {format_data}")
                     return None

                 absolute_path = os.path.join(self.config.save_path, relative_path)
                 if not self.config.overwrite_resources and os.path.exists(absolute_path):
                      logger.debug(f"[{topic_data.topic_id}/{resource_field_name}] Ресурс уже существует (уточненный путь: {relative_path}), overwrite_resources=False.")
                      return relative_path
            ...
            logger.error(f"[{topic_data.topic_id}/{resource_field_name}] Ошибка ввода-вывода при сохранении ресурса {log_path}: {e}")
            return None
        except Exception as e:
            logger.exception(f"[{topic_data.topic_id}/{resource_field_name}] Неожиданная ошибка при обработке ресурса {resource_url}: {e}")
            return None
...
class ScraperApp:
    ...
    def _safe_close_playwright(self):
        ...
        if self.playwright:
            ...
            except Exception as e: logger.error(f"Ошибка при остановке Playwright: {e}")
    ...
    def _parse_field(self, soup: BeautifulSoup, field_conf: FieldConfig, base_url: str, topic_id: str) -> Any:
        ...
        if not elements:
            if field_conf.required:
                logger.error(f"[{topic_id}/{field_name}] Обязательное поле не найдено (селектор: {field_conf.selector})")
        ...
        if field_conf.type == 'text':
                ...
                if field_conf.filter_regex:
                    if field_conf.filter_regex not in self.filter_regex_cache:
                        try:
                            ...
                            logger.debug(f"Скомпилировано regex для '{field_name}': {field_conf.filter_regex}")
                             ...
                             logger.error(f"[{topic_id}/{field_name}] Ошибка компиляции regex '{field_conf.filter_regex}': {e}. Фильтрация отключена.")
                    ...
                    if compiled_regex:
                        ...
                        if original_count != filtered_count:
                            logger.debug(f"[{topic_id}/{field_name}] Отфильтровано {original_count - filtered_count}/{original_count} элементов regex: {field_conf.filter_regex}")
                    else:
                         logger.warning(f"[{topic_id}/{field_name}] Не удалось применить фильтрацию (ошибка компиляции regex): {field_conf.filter_regex}")
            ...
            if not resource_relative_url:
                logger.warning(f"[{topic_id}/{field_name}] Атрибут '{attr_name}' пуст для селектора {field_conf.selector}")
                return None
    ...
    def _process_topic_page(self, topic_url: str):
        ...
        if not self._navigate_to(topic_url):
            ...
            if not self._navigate_to(topic_url):
                logger.error(f"Вторая попытка перейти на страницу топика также не удалась: {topic_url}. Пропуск топика.")
                return
        ...
        if len(html_content) < self.config.min_topic_content_lent:
            logger.warning(f"Контент страницы топика слишком короткий ({len(html_content)} байт, < {self.config.min_topic_content_lent}): {topic_url}")
        ...
        try:
                 ...
                 try:
                     ...
                     if field_conf.type == 'resource_url' and value:
                         ...
                         if ext in field_conf.exclude_extensions:
                             logger.debug(f"[{topic_id}/{field_name}] Исключаем URL ресурса с расширением {ext}: {value}")
                             value = None
                     ...
                     logger.error(f"Ошибка парсинга обязательного поля '{field_name}' для {topic_url}: {e}. Топик пропущен.")
                     return
                 except Exception as e:
            ...
            if self.resource_downloader and topic_data.resource_urls_to_download:
                logger.debug(f"[{topic_id}] Найдено {len(topic_data.resource_urls_to_download)} URL ресурсов для скачивания.")
            ...
            try:
                ...
                while current_url:
                    ...
                    if not self._navigate_to(current_url):
                        logger.error(f"Не удалось загрузить страницу списка: {current_url}. Прерывание для {start_url}.")
                        break # Выход из while current_url

                    # Используем обертку для получения контента
                    html_content = self._get_page_content()
                    if not html_content:
                        logger.error(f"Не удалось получить контент страницы списка: {current_url}. Прерывание для {start_url}.")
                        break # Выход из while current_url
                    ...
                    for preview in previews:
                         ...
                         if link_tag and link_tag.get('href'):
                             ...
                             logger.warning(f"Не удалось найти ссылку на топик в превью (селектор: {self.config.selectors['topic_link']})")
                    ...
                    for i, topic_url in enumerate(topic_urls_on_page):
                         logger.info(f"--- Обработка топика [{i+1}/{len(topic_urls_on_page)}] со страницы {page_num} ---")
                    ...
                    if next_page_tag and next_page_tag.get('href'):
                        ...
                        if next_page_relative_url.startswith(('javascript:', '#')) or not next_page_relative_url:
                            ...
                            logger.info(f"Ссылка на следующую страницу не найдена для {start_url}. Завершение пагинации для этого URL.")
                            current_url = None
